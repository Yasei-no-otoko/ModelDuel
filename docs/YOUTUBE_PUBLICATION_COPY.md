# YouTube publication copy

This file contains copy only. Uploading, publishing, visibility selection, and
the final public URL remain user-owned actions; Codex has not published a video.

## Title

```text
ModelDuel — Two Models Predict. Evidence Decides. | OpenAI Build Week 2026
```

## Description

```text
ModelDuel turns a learner’s explanation or sketch into two runnable 3D worlds—their mental model and the scientific model—then asks them to predict before the evidence is revealed.

Education entry for OpenAI Build Week 2026. Built with Codex and GPT-5.6 Terra/Luna, Structured Outputs, validated tool orchestration, deterministic Three.js simulations, and Cloudflare Workers.

Live demo: https://modelduel.yasei.workers.dev

This video demonstrates the free verified-sample path. AI-generated narration: OpenAI TTS (nova). No copyrighted music.

#OpenAI #Codex #Education #ThreeJS
```
